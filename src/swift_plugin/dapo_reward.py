...
class DSTRewardIncrementalNoTranscript(ORM):
    """Ablation: incremental DST reward without transcript score.

    Reward = w_diff_f1 * diff_f1 + w_exact * exact_match + w_format * format_score

    Default weights: diff_f1=0.7, exact_match=0.15, format=0.15
    """
    ...
    def __call__(self, completions: List[str], solution: List[str] = None,
        ...
        DSTRewardIncrementalNoTranscript._call_count += 1
        is_rank0 = int(os.environ.get('LOCAL_RANK', '0')) == 0
        if is_rank0 and DSTRewardIncrementalNoTranscript._call_count % self.LOG_INTERVAL == 0:
            self._log_samples(completions, solution, rewards, breakdowns)
    ...
    def _log_samples(
        ...
        print(f'\n{sep}', flush=True)
        print(
            f'[DST Reward NoTranscript] call #{DSTRewardIncrementalNoTranscript._call_count}  '
            f'num_completions={len(completions)}', flush=True)
        print(sep, flush=True)
...
def parse_state_json(answer_text: str) -> Dict:
    ...
    Returns an empty dict if the answer is empty or not valid JSON.
    """
    if not answer_text or not answer_text.strip():
        return {}
    try:
...
def compute_state_slot_f1(pred_state: Dict, gold_state: Dict) -> float:
    """Compute slot-level F1 between two belief states."""
...
def compute_fullstate_format_score(response: str) -> float:
    """Evaluate format correctness for full-state DST output.

    Checks for:
    - <transcript>...</transcript> tags (0.3)
    - <answer>...</answer> tags (0.3)
    - Valid JSON in answer content (0.4)
    """
    score = 0.0
    ...
    if has_answer:
        ...
        if answer_content is not None:
            ...
            if not stripped:
                # Empty answer -> valid (empty state = {})
                score += 0.4
            else:
                try:
...
# ---------------------------------------------------------------------------
# Full-state DST ORM Reward Class
# ---------------------------------------------------------------------------


class DSTRewardFullState(ORM):
    """Combined full-state DST reward function.

    Reward = w_transcript * transcript_wer_reward
           + w_slot_f1   * state_slot_f1
           + w_jga       * jga (exact match)
           + w_format    * format_score

    Default weights: transcript=0.3, slot_f1=0.4, jga=0.2, format=0.1
    """
    ...
    def __call__(self, completions: List[str], solution: List[str] = None,
        ...
        DSTRewardFullState._call_count += 1
        is_rank0 = int(os.environ.get('LOCAL_RANK', '0')) == 0
        if is_rank0 and DSTRewardFullState._call_count % self.LOG_INTERVAL == 0:
            self._log_samples(completions, solution, rewards, breakdowns)
    ...
    def _log_samples(
        ...
        print(f'\n{sep}', flush=True)
        print(
            f'[FullState DST Reward] call #{DSTRewardFullState._call_count}  '
            f'num_completions={len(completions)}', flush=True)
        print(sep, flush=True)
    ...
    def _score_single(
        ...
        # --- Format score ---
        format_score = compute_fullstate_format_score(completion)
        ...
        pred_answer = extract_answer(completion)
        if pred_answer is not None:
            pred_state = parse_state_json(pred_answer)

            # Get gold state
            gold_state = {}
            if belief_state_str:
                try:
                    gold_state = json.loads(belief_state_str)
                    if not isinstance(gold_state, dict):
                        gold_state = {}
                except (json.JSONDecodeError, TypeError):
                    gold_state = {}

            slot_f1 = compute_state_slot_f1(pred_state, gold_state)
            jga = 1.0 if pred_state == gold_state else 0.0

        # --- Total reward ---
        total = (
            weights['transcript'] * transcript_score
            + weights['slot_f1'] * slot_f1
            + weights['jga'] * jga
            + weights['format'] * format_score
        )
...
orms['dst_incremental_no_transcript'] = DSTRewardIncrementalNoTranscript
orms['dst_fullstate'] = DSTRewardFullState
